...
resource "aws_db_instance" "mysql" {
  ...
  name                      = "worker_db"
  username                  = "worker"          # can change to suit you
  password                  = "ToptoBottom123$" # can change to suit you
  port                      = "3306"
  db_subnet_group_name      = aws_db_subnet_group.mysql-subnet-group.name
  vpc_security_group_ids    = [aws_security_group.rds_secgrp.id, aws_security_group.ecs_sg.id]
